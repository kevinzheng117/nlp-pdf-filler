...
export async function extractFields(text) {
  try {
    ...
    // Step 3: Calculate overall confidence with new scoring rules
    ...
    // +0.1 if both buyer & seller found via rules (regex)
    const buyerMethod = finalResults.buyer?.method || 'none';
    const sellerMethod = finalResults.seller?.method || 'none';
    if (buyerMethod === 'regex' && sellerMethod === 'regex' && 
        finalResults.buyer?.value && finalResults.seller?.value) {
